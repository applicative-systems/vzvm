/*
  Backend-neutral half of the Linux remote-builder profile.

  Everything in nixpkgs' `nixos/modules/profiles/nix-builder-vm.nix` that does
  not depend on the VM backend: the `virtualisation.darwin-builder` options,
  guest user and SSH setup, Nix settings, and the `macos-builder-installer`.

  nix-builder-vm.nix     = this + virtualisation/qemu-vm.nix   (upstream, QEMU)
  nix-builder-vz-vm.nix  = this + virtualisation/vz-vm.nix     (this repo, vz)

  Installer uses backend through `config.system.build.vm`, which both backends provide.

  DO NOT AUTO-FORMAT: keep close to upstream original!
*/
{
  config,
  lib,
...

{
  ...
  config = {
    ...
    # Allow the system derivation to be substituted, so that users are less likely to
    # run into a state where they need the builder running to build the builder.
    ...
    virtualisation = {
      ...
      #
      # The vz backend always boots from a store image, so unlike the QEMU backend
      # there is no `useNixStoreImage` toggle to set here.
